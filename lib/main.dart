...
import 'package:pirpg/ui/battle_screen.dart';
        ...
        builder: (context, auth, _) {
          if (auth.isLoading) {
            return const Scaffold(
              body: Center(
                child: CircularProgressIndicator(color: Color(0xFFF0E68C)),
              ),
            );
          }
...
class _GameScreenState extends State<GameScreen> {
  ...
  void _startPhaseChallenge(String phaseId) {
    ...
    if (playerState.isPhaseDefeated(phaseId)) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('A ameaça desta região já foi eliminada!'),
        ),
      );
      return;
    }

    // Se for a fase H15 (Ruinas da Lógica), abre a tela de batalha
    if (phaseId == 'fase_h15') {
      Navigator.push(
        context,
        MaterialPageRoute(builder: (context) => BattleScreen(phaseId: phaseId)),
      ).then((result) {
        // result é true se venceu, false se perdeu, null se fechou
        if (result == true) {
          playerState.markPhaseDefeated(phaseId);
          playerState.addItem(
            GameItem(
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                '🎉 NÚCLEO DE LÓGICA X Derrotado! Você recebeu uma Carta de Regeneração!',
              ),
              duration: Duration(seconds: 3),
            ),
          );
        } else if (result == false) {
          playerState.takeDamage(30);
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('💀 Você foi derrotado! Perdeu 30 HP!'),
    ...
    // Caso contrário, abre o desafio de perguntas
    int phaseNum = 1;
    ...
    if (questions.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Sábios ainda estão elaborando os desafios desta região!',
          ),
        ),
      );
      return;
      ...
      pageBuilder: (context, anim1, anim2) => QuestionOverlay(
        ...
        onCorrectAnswer: () {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                'Boss Derrotado! Você recebeu uma Carta de Regeneração!',
              ),
            ),
          );
        },
        onWrongAnswer: () {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Ataque do Boss! Você perdeu 50 HP!')),
          );
        },
      ),
      transitionBuilder: (context, anim1, anim2, child) {
  ...
  Widget build(BuildContext context) {
    ...
    final locationName = geoService.isInsideCampus()
        ? 'REINO DA PUC'
        : 'TERRAS SELVAGENS';
    final locationColor = geoService.isInsideCampus()
        ? MedievalColors.emeraldLight
        : MedievalColors.crimsonLight;
    ...
    final healCard = playerState.inventory
        .where((item) => item.name == 'Carta de Regeneração')
        .firstOrNull;
      ...
      body: Stack(
        children: [
          ...
          Positioned(
            ...
            left: 0,
            right: 0,
            child: Center(child: _TavernButton(onTap: _openPause)),
          ),
              ...
              child: GestureDetector(
                onTap: () {
                  if (playerState.hp >= playerState.maxHp) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('Seu HP já está cheio!')),
                    );
                    return;
                  }
                  playerState.heal(75);
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text('Curado em 75 HP!')),
                  );
                },
                child: Container(
                  padding: const EdgeInsets.symmetric(
          ...
          Column(
            ...
            children: [
              Text(
                '$hp / $maxHp HP',
                style: const TextStyle(
                  color: MedievalColors.crimsonLight,
                  fontWeight: FontWeight.bold,
                  fontSize: 14,
                ),
              ),
            ...
            child: Text(
              ...
              style: TextStyle(
                color: isUnlocked
                    ...
                    : MedievalColors.textMuted.withAlpha(100),
                fontSize: 11,
